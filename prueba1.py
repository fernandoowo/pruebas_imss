...
N8N_WEBHOOK_URL = "https://f7c4-187-189-50-136.ngrok-free.app/webhook/fa833c9e-f44a-44e5-9515-378505671975"
#N8N_WEBHOOK_URL = "https://f7c4-187-189-50-136.ngrok-free.app/webhook-test/fa833c9e-f44a-44e5-9515-378505671975"
...
# Nueva ruta para procesar audio directamente
@app.route('/consultar-audio', methods=['POST'])
@handle_errors
def consultar_audio():
    try:
        # Asumimos que viene un archivo de audio
        if 'audio' not in request.files:
            return jsonify({"error": "No se encontró archivo de audio"}), 400
            
        # Aquí procesaríamos el audio con un servicio de reconocimiento de voz
        # Por ahora, simulamos un texto reconocido
        # En una implementación real, usarías algo como:
        # - Google Speech-to-Text API
        # - Azure Speech Services
        # - Whisper API de OpenAI
        
        # Simulación de texto reconocido (reemplazar con la implementación real)
        mensaje = "Texto simulado reconocido del audio"
        
        # Usar la misma lógica de consulta que la ruta /consultar
        payload = {
            "mensaje": mensaje,
            "metadata": {
                "origen": "asistente-imss-web-voz",
                "version": "1.0",
                "requiere_audio": True
            }
        }
        
        # El resto del código sería similar a /consultar
        response = requests.post(
            N8N_WEBHOOK_URL,
            headers={
                'Content-Type': 'application/json',
